...
def resolve_refs(inputs: Inputs, env: Dict[str, str], event: Dict,
    ...
    has moved on.
    """
    event_name = env.get('DOCX_EVENT_NAME', env.get('GITHUB_EVENT_NAME', ''))
...
def render_snapshot(inputs: Inputs, change: Change, data: bytes,
                    previewer: Optional[str]) -> None:
    """Show an added/deleted file (or pure rename), without claiming it is a diff."""
    document_path, html_path = redline_output_paths(inputs.output_dir, change.path)
    document_path.parent.mkdir(parents=True, exist_ok=True)
    document_path.write_bytes(data)
    change.document = document_path.as_posix()
    if previewer and generate_preview(previewer, document_path, html_path):
        change.html = html_path.as_posix()
    elif previewer and inputs.html_preview == 'true':
        change.error = 'Required HTML preview could not be generated.'
...
def write_manifest(changes: List[Change], inputs: Inputs, base: Optional[str],
                   head: Optional[str], env: Dict[str, str]) -> None:
    """Persist all file results, including unpaired/failed files, with provenance."""
    directory = Path(inputs.output_dir).resolve()
...
def load_event(env: Dict[str, str]) -> Dict:
    path = env.get('DOCX_EVENT_PATH') or env.get('GITHUB_EVENT_PATH')
        ...
        for change in changes:
            try:
                if inputs.mode == 'latest':
                    ...
                    if inputs.render_unpaired and original == modified:
                        render_snapshot(inputs, change, modified, previewer)
            ...
            print(f'  {change.path}: {change.revisions} revision(s) -> {change.redline}')
